...
def test_match_event(caplog):
    ...
    assert glmh.match_event_in_sublist("Something has happened", 0) == 'root'
    assert glmh.match_event_in_sublist("Something else has happened", 0) == 'Alice'
    assert glmh.match_event_in_sublist("Something that has not happened", 0) is None
